...
KKT 조건으로 인해 w도 α에 대한 식으로 나오기 때문에 α에 대한 최적화만 진행하면 된다. <br>
이때 모든 데이터의 α에 대해 KKT에 만족하지못한 α만 최적화를 진행하여 fitting을 시킨다 <br>
다음 학습함수는 다음과 같이 업데이트를 진행한다
    ...
    while passes < self.conv:
        ...
        for i in range(n):
            ...
            if ((y[i]*E_i < -self.tol and self.alpha[i] < self.C) or
                ...
                # α를 box constraints에 맞춰 KKT의 0 ≤ α ≤ C에 맞춰준다
                if y[i] != y[j]:
